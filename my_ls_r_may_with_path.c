...
char read_all_files (DIR *dir, struct dirent **dirp, char *all_path)
{
    int index = 0;
    if (dir == NULL)
        return 84;
...
char read_all_folders (DIR *dir, struct dirent *dirp[10000],
...
char my_ls_r_may_with_path (int ac, char **av)
{
    ...
    if (dir == NULL)
        return 84;
    path_length = my_str_count_len(av[2]) + 300;
    all_path = malloc(path_length);
    if (all_path == NULL)
        return 84;
    read_all_files(dir, dirp, all_path);
    dir = opendir(av[2]);
    read_all_folders (dir, dirp, all_path, av[2]);
    free(all_path);
    closedir(dir);
    return 0;
}

...
char read_files (DIR *dir, struct dirent **dirp, char *all_path)
{
    int index = 0;
    if (dir == NULL)
        return 84;
...
char read_folders (DIR *dir, struct dirent *dirp[10000],
...
char my_ls_r_may (const char *path)
{
    DIR *dir;
    ...
    if (all_path == NULL)
        return 84;
    read_files(dir, dirp, all_path);
    dir = opendir(path);
    read_folders (dir, dirp, all_path, path);
    free(all_path);
    closedir(dir);
    return 0;
}
